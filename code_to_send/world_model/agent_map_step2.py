...
# Minecraft default FOV is 70°. With a 1:1 video aspect (64x64) the
# horizontal FOV equals the vertical FOV, so 70° works for the column->yaw
# correction below.
FOV_DEG = 70.0
...
OUT_DIR = "world_model_out/May7th"
...
def mark_current_free(grid, x, z):
    """Always mark the agent's current cell FREE.

    If the cell was previously marked DIAMOND, overwrite it: the agent is
    physically standing here and the mission hasn't ended, so the DIAMOND
    mark must have been a stale ray-cast guess. Letting it persist would
    trap the planner ("walk to cell I'm already on") forever.
    """
    gx, gz = world_to_grid(x, z)
    grid[gz, gx] = FREE

def maybe_mark_diamond(grid, x, z, yaw, frame, diamond_visible):
    """Ray-cast in the *actual* direction of the cyan pixels and mark
    the farthest unknown cell along that ray as DIAMOND.

    The previous version cast straight along the agent's yaw, which
    was wrong whenever the diamond was off-center in the FOV (e.g. at
    yaw=270 the diamond at world (4, 4) is 41° to the right, not
    directly east). We compute the cyan centroid's column in the frame,
    convert it to an angular offset within the FOV, and project a ray
    at (yaw + offset). With 70° FOV: column 0 is yaw-35, column 63 is
    yaw+35 (right of center is +yaw because Minecraft yaw increases
    clockwise from above and image columns increase to the agent's right).
    """
    if not diamond_visible:
        return

    mask = diamond_pixel_mask(frame)
    if mask.sum() < CYAN_MIN_PIXELS:
        return

    # cyan centroid column -> angular offset within the FOV
    ...
    # Minimum ray distance before we trust a cell as DIAMOND. If an obstacle
    # is just 1-2 cells away, the cell right before it is much more likely
    # to be a "ray bounced off an obstacle" artifact than the real diamond.
    # Require the diamond to be at least MIN_RAY_K cells away from the agent.
    MIN_RAY_K = 3
...
def maybe_mark_obstacle(grid, prev_x, prev_z, prev_yaw, prev_action, cur_x, cur_z):
    ...
    A confirmed bump is the strongest evidence we have about a cell — it is
    physically blocked. ALWAYS overwrite, including over a previous
    DIAMOND or FREE mark. (DIAMOND marks come from speculative ray casts
    and can land on cyan-coloured non-diamond blocks like emerald; a real
    bump trumps that. FREE shouldn't normally co-occur with a bump in env3
    which is static, but if it does the bump is the truth.)
    """
...
def planned_action(grid, current_gxgz, current_yaw, stuck_count, prev_action):
    """Pick an action using A* on the current map.

    Goal selection (in order of preference):
      1. Try every cell currently marked DIAMOND, sorted by Manhattan
         distance to the agent. Some marks may be wrong (the centroid
         ray-cast is approximate), so if A* can't reach one we try the
         next instead of giving up.
      2. If no DIAMOND cell is reachable, fall back to the nearest
         frontier cell (unknown adjacent to free) so the agent keeps
         exploring rather than spinning in place.
      3. If even frontier exploration is stuck, do a reactive escape.
    """
    cgx, cgz = current_gxgz
    target_label = None
    path = None

    # 1) try every diamond cell, closest first
    diamond_locs = np.argwhere(grid == DIAMOND)
    if len(diamond_locs) > 0:
        ordered = sorted(
            ((int(gx), int(gz)) for gz, gx in diamond_locs),
            key=lambda c: abs(c[0] - cgx) + abs(c[1] - cgz),
        )
        for goal in ordered:
            if goal == current_gxgz:
                # we're standing on a marked DIAMOND cell but mission isn't
                # ending — the mark is stale (mark_current_free will clear
                # it next iteration). Skip and try other diamond cells.
                continue
            p = a_star(grid, current_gxgz, goal)
            if p:
                path = p
                target_label = "DIAMOND"
                break

    # 2) fall back to frontier exploration
    if path is None:
        frontier_goal = find_frontier_cell(grid, current_gxgz)
        if frontier_goal is not None and frontier_goal != current_gxgz:
            p = a_star(grid, current_gxgz, frontier_goal)
            if p:
                path = p
                target_label = "frontier"

    # 3) nothing to plan to: reactive escape
...
def main():
    ...
    # Pre-mark the four perimeter rows/cols as OBSTACLE. These cells
    # correspond to the env3 stone walls (cobblestone N, planks S,
    # brick W, glass E) which the agent's bounding box can never enter.
    # Without this, A* will happily plan paths that include these
    # unreachable cells, and the ray-cast can mark false-positive
    # cyan readings at the wall as DIAMOND, trapping the planner.
    grid[0, :] = OBSTACLE
    ...
    for step in range(MAX_STEPS):
        ...
        maybe_mark_diamond(grid, x, z_pos, yaw, frame, diamond_visible)
        ...
        # Backup: if we've been stuck for a long time, the marked DIAMOND
        # cells are almost certainly wrong (probably wandered into the
        # emerald block or a glass-edge artifact). Clear them and let the
        # planner fall back to frontier exploration; new ray casts from
        # different angles will mark a better cell.
